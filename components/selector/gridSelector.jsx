...
class GridSelector extends React.Component {
  ...
  render() {
    return (<Box sx={{ backgroundColor: 'rgb(247, 247, 247)', minWidth: '500px', width: '80vw', pt: '10px' }}>
        <Card sx={{ m: '20px' }}>
          {
            this.renderHeader()
          }
        ...
        <DataGridContainer
          onRowSelectedChanged={(row) => { this.setState({ value: row !== null ? row : null }) }}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', m: '10px' }}>
          ...
          <Button onClick={this.onClose} sx={{ ml: '2px' }} variant='outlined'>Cancel</Button>
        </Box>
      </Box>
    );
  }
